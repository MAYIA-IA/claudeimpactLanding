...
const intelligence = new CopilotKitIntelligence({
  ...
  apiUrl: process.env.INTELLIGENCE_API_URL ?? "http://localhost:4218",
  wsUrl: process.env.INTELLIGENCE_GATEWAY_WS_URL ?? "ws://localhost:4418",
});
...
app.use("*", async (c, next) => {
  console.log(`[BFF] Request: ${c.req.method} ${c.req.url}`);
  // console.log(`[BFF] Headers: ${JSON.stringify(c.req.header())}`);
  await next();
  console.log(`[BFF] Response: ${c.res.status}`);
});

// Add a simple health check
app.get("/api/health", (c) => c.json({ status: "ok" }));

// Debug endpoint for runtime
app.get("/api/copilotkit/debug", (c) => {
  return c.json({
    status: "ok",
...
// Use the official Hono handler for CopilotKit
const handler = createCopilotHonoHandler({
  runtime,
  basePath: "",
});

app.route("/api/copilotkit", handler);
